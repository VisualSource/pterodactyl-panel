...
function EditContainer(){
    ...
    const submit = ({ server }: Values, { setSubmitting }: FormikHelpers<Values>) => {
        ...
        .then(()=>setDomain({ ...domain, server_id: server?.id ?? null, server: server ? { id: server.id, name: server.name, identifier: server.identifier } : null }))
        .catch(error=>{
            console.error(error);
                            ...
                            <div css={tw`w-full flex flex-row items-center mt-6`}>
                                <div css={tw`flex`}>
                                    <DomainDeleteButton
                                        domainId={domain.id}
                                        onDeleted={() => {
                                            addFlash({ key: "domains", message: `Subdomain (${domain.domain}) will be completly removed in 2-5 minutes.`, type: "success" });
