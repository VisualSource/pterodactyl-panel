    ...
    return (
        <AdminBox icon={faCogs} title="Settings" isLoading={isSubmitting}>
             <div css={tw`grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6`}>
                <Field defaultValue={initialValues?.external_port} id='external_port' name='external_port' label='External Port' type='number'/>
                <div>
                    <Label htmlFor="type">Network Type</Label>
                    <Select id="type" name="type" defaultValue={initialValues?.type ?? "both"} onChange={(ev)=>setFieldValue("type",ev.target.value)}>
                ...
                { !nodes.error || !nodes.isLoading ? (<div>
                ...
                <div>
                    ...
                    <Select defaultValue={initialValues?.allocation_id} id="allocation_id" name="allocation_id" onChange={(ev)=>setFieldValue("allocation_id",parseInt(ev.target.value))}>
                        { !allocations.error || !allocations.isLoading ? (
                            <>
                                <option>Select Allocaiton</option>
                                { allocations.data?.map((value,key)=>(
                                    <option key={key} value={value.id}>{value.getDisplayText()}</option>
                                )) }
                            </>
                        ) : (
